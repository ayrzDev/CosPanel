          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <textarea value={formData.condition} onChange={(e) => setFormData({...formData, condition: e.target.value})} className="w-full px-3 py-2 border rounded-lg" rows={3} placeholder='{"field":"subject","operator":"contains","value":"spam"}' required />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Action (JSON)</label>
                <textarea value={formData.action} onChange={(e) => setFormData({...formData, action: e.target.value})} className="w-full px-3 py-2 border rounded-lg" rows={3} placeholder='{"type":"delete"}' required />
              </div>
